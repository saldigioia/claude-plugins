...
echo
echo "== 4. THE INVARIANT: a printed Tier-1 command must not refuse at pre-flight =="
# Generalizes B1 / F12 / this defect: whatever the clinic hands the operator as
# "ready to run", running it must not come straight back as a pre-flight refusal.
i=0
for prof in none pair full2x; do
  i=$((i+1))
  case "$prof" in
    none)   env_file="";;
    pair)   env_file="$WORK/pair.csv";;
    full2x) env_file="$WORK/full2x.csv";;
  esac
  if [ -n "$env_file" ]; then c=$(PF_PKT_FILE="$env_file" bash "$SC/clean.sh" "$S" 2>&1)
  else c=$(bash "$SC/clean.sh" "$S" 2>&1); fi
  case "$c" in
    *"$READY"*)
      if [ -n "$env_file" ]; then
        PF_PKT_FILE="$env_file" bash "$SC/zero-base.sh" "$S" "$WORK/inv$i.ts" >/dev/null 2>&1; zrc=$?
      else
        bash "$SC/zero-base.sh" "$S" "$WORK/inv$i.ts" >/dev/null 2>&1; zrc=$?
      fi
      [ "$zrc" -ne 2 ] \
        && ok "profile=$prof: clinic offered the route and zero-base did not refuse it (rc=$zrc)" \
        || no "profile=$prof: clinic offered a route that refuses at pre-flight (rc=2) — the F12 class"
      ;;
    *) ok "profile=$prof: clinic did not offer the route (nothing to contradict)";;
  esac
done
...
grep -q -- '--preflight-only' "$SC/clean.sh" \
