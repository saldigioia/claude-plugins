...
**The cut bench (1.19.0, REMUX-PIPELINE retrace).** When a build fails and
candidate repairs exist, prove each candidate on a ~30-second copy-cut of the
source BEFORE committing the full file: each route then costs seconds, and
the full `verify.sh` suite runs on every candidate's cut output. The retrace's
R3 route *played perfectly* and was caught only by verify on a 25 MB cut — a
visual check would have shipped it. Two corollaries from the same session:
**never route through a raw elementary-stream intermediate** to fix a
container problem (the raw file discards container timestamps — that, not
Annex-B conversion, is what restamps; an inline bsf in the same mux leaves
the PTS column bit-equal, measured max |Δ| 0.000000), and **a fix that works
diagnostically is not the deliverable** (hev1 wrote the correct config and
still rendered no frame in AVFoundation — its value was isolating the
mechanism).
...
| Broadcast splice changes resolution mid-stream (new SPS / MPEG-2 sequence header) | **Detector implemented (1.15; `--headers` fast mode 1.19.0)**: `scripts/dim-scan.sh IN --headers` answers the static-dimensions question **demux-only in seconds** (distinct SPS / sequence-header census — one distinct PS proves the dimensions cannot change, since they live there; >1 routes to the decode mode to confirm, `DIMSCAN_HDR` machine line). `scripts/dim-scan.sh IN` is the decode-mode sign-off — whole-file frame-dimension sweep (a decode pass, background-able; `clean.sh --deep` runs it) counts the changes and names each splice PTS. The downstream blindness is unchanged and measured (2026-08-14): the copy mux succeeds with no warning, `stsd` declares the FIRST resolution while later samples decode at the new size, and no mux/verify gate catches it — so the routes are source-domain: cut at the splice (`surgical-cut.sh` on TS) or do NOT remux across the junction. `references/known-limits.md` |
...
| HEVC MKV builds a `.mov` that won't decode at all (ffprobe: "no video stream" / `Invalid data`) | The **PS-less hvcC stub** class (1.19.0): the MKV's CodecPrivate has `numOfArrays=0`, parameter sets in-band; `hvc1` + stub = ffmpeg silently writes an EMPTY `hvcC`. The builders now detect and reroute via the inline `hevc_mp4toannexb` bsf automatically (`RMX_HVCC route=annexb-bsf`, timestamps untouched), and `mux_census` refuses any QTFF part with empty h264/hevc extradata (`dcfg=empty`). Do NOT reach for `hev1` (writes the config but AVFoundation renders no frame, measured 2026-09-01) or a raw-elementary rebuild (restamps). `references/known-limits.md` has the class + the manual surgery fallback |
...
| `RMX_CENSUS` | every builder, post-mux, pre-bless (`lib-mux.sh`) | `stage=remux\|dual-track\|resync\|rebuild-paff\|pairfill-paff\|derive-dts\|trim-to-idr\|metadata\|rung4\|zero-base\|surgical-cut planned=<N> written=<M> codecs=ok\|mismatch\|na match=ok\|MISMATCH dcfg=ok\|empty\|na` (D5, 1.13 — the plan-vs-file reconciliation; MISMATCH is exit 1 with the artifact kept as `.part`; the two clinic stage values appended 1.15; `derive-dts` emitted since 1.14, missing from this enum until CHECKUP-2026-08-27 F4; `dcfg=` appended 1.19.0 — the decoder-config census: a QTFF part whose h264/hevc extradata is missing/empty is exit 1 with `dcfg=empty`, the empty-hvcC class) |
| `RMX_HVCC` | `remux.sh`/`dual-track.sh`/`resync.sh` pre-mux (1.19.0) | `route=annexb-bsf reason=ps-stub` — the PS-less hvcC stub reroute: HEVC source whose extradata has `numOfArrays=0` gets the inline `hevc_mp4toannexb` bsf in the same mux (timestamps untouched, measured max \|Δ\| 0.000000) so movenc rebuilds the config from the in-band parameter sets |
| `DIMSCAN_HDR` | `dim-scan.sh --headers` (1.19.0) | `mode=headers codec= distinct_ps= verdict=clean\|findings\|unmeasured` — the demux-only distinct-parameter-set census (H.264/HEVC SPS, MPEG-2 sequence header); `clean` (=1) proves static coded dimensions at ~20× less cost than the decode sweep; `findings` (>1) routes to the decode mode to confirm what changed |
