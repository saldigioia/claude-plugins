...
1. **Probe** the source first — never guess:
   ...
   ffmpeg-version warnings (DV/colr/MP2 behavior differs by version).
...
3. **Verify** every output before trusting it: `scripts/verify.sh SOURCE OUTPUT`.
   ...
   pipeline/source type. Never default to a full double decode.
...
Rung 3  Rebuild timeline from the elementary stream
        ...
        forced by: field-coded (PAFF) H.264 too broken for genpts. Video still
        bit-identical; access units + timestamps re-derived at the field rate.
...
non-monotonic DTS → Rung 3. Detail and the manual commands live in
