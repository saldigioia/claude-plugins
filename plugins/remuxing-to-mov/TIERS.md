...
| T3.4 | derive-dts `_pair_parity` evidence rule — "fields are coded-adjacent one **field duration** apart", 99% bar | `derive-dts.py` | **EVIDENCE REPLACED, standard kept (1.16.0).** The rule measured *timestamp deltas* and inferred *structure* — a proxy, while the direct evidence sat unread in every slice header. Pairing is now structural (`field_pic_flag=1`, `bottom_field_flag` 0→1, same `frame_num`, coded-adjacent) and timestamps come from POC (`k = POC + C`, C constant per `(IDR-epoch, bottom_field_flag)`, ≥99.9% unanimity over ≥100 samples). The unforgiving bar stays. Tests 100, 105. |
| T3.5 | derive-dts duplicate-PTS refusal | `derive-dts.sh:113,:167`, `derive-dts.py` | **CONVERTED (1.16.0)**: each duplicate is adjudicated by POC — the later holder never fits its local lattice, the earlier always does. Refuses only a duplicate POC cannot adjudicate, naming the per-duplicate evidence. Every windowed number is scoped at the point of print. Tests 101, 106. |
