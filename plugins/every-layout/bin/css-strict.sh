...
#
# Usage:
#   bin/css-strict.sh [directory|file.css]             # strict mode (default)
#   bin/css-strict.sh --archival [directory|file.css]  # adds ELA_006 checks
# A single .css file may be passed for targeted fixture testing.
#
# Exit codes:
...
if [ -f "$DIR" ]; then
  case "$DIR" in
    *.css) ;;
    *) echo "error: $DIR is not a .css file" >&2; exit 2 ;;
  esac
elif [ ! -d "$DIR" ]; then
  echo "error: $DIR is not a .css file or directory" >&2; exit 2
fi
